...
export const POST = authenticatedHandler('save daily balance', async (request, { userId }) => {
  const body = await request.json();
  const { date, balance, income, expense } = body;
...
// GET /api/daily-balance?days=5 - 최근 N일 잔액 조회
// GET /api/daily-balance?year=2024&month=1 - 특정 월의 일별 잔액 조회
export const GET = authenticatedHandler('fetch daily balance', async (request, { userId }) => {
