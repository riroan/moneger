...
import { isAssetFormationCategory } from '@/lib/cash-flow';
...
async function computeAndUpsertDailyBalance(
  ...
  const [previousBalance, dayIncomeAgg, dayExpenseAgg, daySavingsAgg, dayInvestmentAgg] = await Promise.all([
    ...
    client.transaction.aggregate({
      where: spendingExpenseWhere({ userId, date: { gte: startOfDay, lte: endOfDay }, deletedAt: null }),
      _sum: { amount: true },
    }),
    client.transaction.aggregate({
    ...
    client.transaction.aggregate({
      where: assetFormationCategoryWhere({ userId, date: { gte: startOfDay, lte: endOfDay }, deletedAt: null }),
      _sum: { amount: true },
    }),
  ]);
  ...
  const savings = (daySavingsAgg._sum.amount || 0) + (dayInvestmentAgg._sum.amount || 0);
 ...
 * savings 필드는 저축 목표 입금 + 투자 납입을 포함한 자산 형성 현금흐름이다.
 */
async function calculateMonthlyDailyBalancesFromTransactions(userId: string, year: number, month: number) {
  ...
  const [
    ...
    prisma.transaction.aggregate({
      where: { userId, date: { lt: startDate }, type: 'INCOME', deletedAt: null },
      _sum: { amount: true },
    }),
    prisma.transaction.aggregate({
    ...
    prisma.transaction.groupBy({
      by: ['date'],
      where: { userId, date: { gte: startDate, lte: endDate }, type: 'INCOME', deletedAt: null },
      _sum: { amount: true },
    }),
    prisma.transaction.groupBy({
      by: ['date'],
      where: spendingExpenseWhere({ userId, date: { gte: startDate, lte: endDate }, deletedAt: null }),
      _sum: { amount: true },
    }),
    prisma.transaction.groupBy({
      by: ['date'],
    ...
    prisma.transaction.groupBy({
      by: ['date'],
      where: assetFormationCategoryWhere({ userId, date: { gte: startDate, lte: endDate }, deletedAt: null }),
      _sum: { amount: true },
    }),
  ]);
  ...
  const prevInvestment = previousInvestmentAgg._sum.amount || 0;
  const initialBalance = prevIncome - prevExpense - prevSavings - prevInvestment;
  ...
  mapGroupedToDaily(investmentGrouped, dailyData, 'savings');
...
export async function calculateDailyBalancesFromTransactions(userId: string, days: number) {
  ...
  const [transactions, previousDailyBalance] = await Promise.all([
    prisma.transaction.findMany({
      ...
      include: { category: { select: { categoryGroup: true } } },
      orderBy: { date: 'asc' },
    }),
  ...
  transactions.forEach((transaction) => {
    ...
    if (dailyData[dateKey]) {
      if (transaction.savingsGoalId || isAssetFormationCategory(transaction.category)) {
