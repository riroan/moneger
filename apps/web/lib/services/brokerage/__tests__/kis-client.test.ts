...
describe('KISClient', () => {
  ...
  it('토큰 캐시: 두 번째 호출은 토큰을 재발급하지 않는다', async () => {
    fetchMock
      .mockResolvedValueOnce(jsonRes(tokenBody)) // token
      .mockResolvedValueOnce(jsonRes(balanceBody)) // snapshot 1
      .mockResolvedValueOnce(jsonRes(balanceBody)); // snapshot 2
    const client = new KISClient(creds);
    const [acct] = await client.listAccounts();
    await client.getAccountSnapshot(acct);
    await client.getAccountSnapshot(acct);
    // token 1회 + balance 2회 = 3 (token 재발급 없음)
  ...
  it('getAccountSnapshot: 보유종목/요약을 정규화하고 0수량을 거른다', async () => {
    fetchMock.mockResolvedValueOnce(jsonRes(tokenBody)).mockResolvedValueOnce(jsonRes(balanceBody));
    const client = new KISClient(creds);
    const [acct] = await client.listAccounts();
